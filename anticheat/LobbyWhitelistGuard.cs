...
public static bool IsLobbyWhitelisted(PlayerControl pc)
        {
            if (IsBuiltInProtected(pc)) return true;
...
private static bool IsLobbyWhitelistedIdentity(string name, string fc, string puid)
        {
            if (IsBuiltInProtectedName(name)) return true;
...
private static bool IsBuiltInProtectedName(string name)
        {
            return false;
        }

public static bool IsBuiltInProtected(string name)
        {
            return IsBuiltInProtectedName(name);
        }

public static bool IsBuiltInProtected(PlayerControl pc)
        {
            return pc != null && pc.Data != null && IsBuiltInProtectedName(pc.Data.PlayerName);
        }

public static bool IsBuiltInProtected(ClientData client)
        {
            if (client == null) return false;
            if (IsBuiltInProtectedName(client.PlayerName)) return true;
            return client.Character != null && IsBuiltInProtected(client.Character);
        }

public static bool IsBuiltInProtected(int clientId)
        {
            ...
            try
            {
                ...
                while (cursor.MoveNext())
                {
                    ...
                    if (data != null && data.Id == clientId)
                        return IsBuiltInProtected(data);
                }
            }
            catch { }
...
public static bool IsProtectedFromAnticheat(PlayerControl pc)
        {
            ...
            if (IsBuiltInProtectedName(pc.Data.PlayerName)) return true;
...
public static bool IsProtectedFromAnticheat(ClientData client)
        {
            ...
            {
                if (client.Character.Data != null && IsBuiltInProtectedName(client.Character.Data.PlayerName)) return true;
...
private static void EnsureLobbyWhitelistLoaded()
        {
            try
            {
                if (string.IsNullOrEmpty(lobbyWhitelistPath))
                    lobbyWhitelistPath = System.IO.Path.Combine(Plugin.AeroFolder, "AeroWhiteList.txt");
