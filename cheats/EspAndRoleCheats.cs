        ...
        public static class RoleManager_SelectRoles_Patch
        {
            public static bool Prefix(RoleManager __instance)
            {
                ...
                try
                {
                    ...
                    AeroMenuGUI.EnsureForcedRoleOptions(allPlayers);
                    ...
                    foreach (var p in allPlayers)
                        if (AeroMenuGUI.IsForcedImp(p) || (AeroMenuGUI.TryGetForcedRole(p, out RoleTypes forcedRole) && impRoleTypes.Contains((int)forcedRole)))
                            impostors.Add(p);

                    if (AeroMenuGUI.IsHideAndSeekMode() && AeroMenuGUI.TryGetForcedHideAndSeekSeekerId(out byte seekerId))
                    {
                        ...
                        if (seeker != null)
                        {
                            ...
                            AeroMenuGUI.SetHideAndSeekSeekerOption(seekerId, numImps);
                        }
                    }
                    ...
                    {
                        if (AeroMenuGUI.TryGetForcedRole(pc, out RoleTypes role) && role != RoleTypes.Crewmate && role != RoleTypes.Impostor)
                        {
                            RoleManager.Instance?.SetRole(pc, role);
                            pc.RpcSetRole(role, false);
                        }
                    }
                    foreach (var pc in allPlayers) AeroMenuGUI.RefreshRoleBehaviour(pc);
                    return false;
                }
                catch { return true; }
            }
...
[HarmonyPatch(typeof(PlayerControl), nameof(PlayerControl.TurnOnProtection))]
        public static class PlayerControl_TurnOnProtection_Patch
        {
            ...
            public static void Prefix(PlayerControl __instance, ref bool visible, int colorId, int guardianPlayerId)
            {
                ...
                if (AeroMenuGUI.seeGhosts || AeroMenuGUI.seeProtections) visible = true;
            }

            public static void RefreshVisibleProtections()
            {
                if (!AeroMenuGUI.seeProtections || PlayerControl.AllPlayerControls == null) return;
...
[HarmonyPatch(typeof(PlayerControl), nameof(PlayerControl.SetRoleInvisibility))]
        public static class PlayerControl_SetRoleInvisibility_SeePhantoms_Patch
        {
            ...
            public static void ApplyVisiblePhantom(PlayerControl player)
            {
                ...
                if (vanished && AeroMenuGUI.seePhantoms)
                {
                    player.Visible = true;
                ...
                else if (vanished && !AeroMenuGUI.seePhantoms && player.invisibilityAlpha == 0.5f)
                {
        ...
        public static class PlayerVisuals_LateUpdate_Patch
        {
            public static void Postfix(PlayerPhysics __instance)
            {
                ...
                try
                {
                    ...
                    if (AeroMenuGUI.seeGhosts && __instance.myPlayer.Data.IsDead && PlayerControl.LocalPlayer != null && !PlayerControl.LocalPlayer.Data.IsDead)
                    {
                    ...
                    {
                        string espName = AeroMenuGUI.GetESPNameTag(__instance.myPlayer.Data, outfit.PlayerName);
                        if (!string.Equals(cosmetics.nameText.text, espName, StringComparison.Ordinal))
                            cosmetics.SetName(espName);
                        if (AeroMenuGUI.seeRoles && AeroMenuGUI.showPlayerInfo) cosmetics.nameText.transform.localPosition = new Vector3(0f, 0.186f, 0f);
                        else if (AeroMenuGUI.seeRoles || AeroMenuGUI.showPlayerInfo) cosmetics.nameText.transform.localPosition = new Vector3(0f, 0.093f, 0f);
            ...
            public static void Postfix(MeetingHud __instance)
            {
                try
                {
                    ...
                    foreach (var state in __instance.playerStates)
                    {
                        ...
                        var data = GameData.Instance.GetPlayerById((byte)state.PlayerId);
                        if (data != null && !data.Disconnected && data.DefaultOutfit != null && state.NameText != null)
                        {
                            string espName = AeroMenuGUI.GetESPNameTag(data, data.DefaultOutfit.PlayerName ?? "???");
                            if (!AeroMenuGUI.seeRoles && AeroMenuGUI.revealMeetingRoles && data.Role != null)
                            {
                                ...
                                Color customColor = AeroMenuGUI.GetRoleColor(roleId, data.Role.TeamColor);
                            ...
                            bool showingExtra = AeroMenuGUI.seeRoles || AeroMenuGUI.revealMeetingRoles;
...
[HarmonyPatch(typeof(ChatBubble), nameof(ChatBubble.SetName))]
        public static class ChatBubble_SetName_Patch
        {
            ...
            {
                if (!AeroMenuGUI.showPlayerInfo || bubble == null || bubble.playerInfo == null || bubble.NameText == null) return;
                try
                {
                    string accentHex = AeroMenuGUI.GetEspAccentHex(bubble.playerInfo.PlayerId);
                    string espLine = AeroMenuGUI.BuildESPInfoLine(bubble.playerInfo, 9, false);
...
[HarmonyPatch(typeof(HudManager), nameof(HudManager.Update))]
        public static class FullBright_Patch
        {
            public static void Postfix(HudManager __instance)
            {
                try
                {
                    bool hudModalActive = AeroMenuGUI.IsHudModalActive();
                    ...
                    __instance.ShadowQuad.gameObject.SetActive(
                        !AeroMenuGUI.fullBright && !AeroMenuGUI.zoomOwnsCamera);
                }
                catch { }
            }
        }
        ...
        public static class HudManager_Update_Patch
        {
            ...
            public static void Postfix(HudManager __instance)
            {
                try
                {
                    ...
                    if (!hudModalActive && AeroMenuGUI.alwaysChat && __instance.Chat != null && __instance.Chat.gameObject != null)
                        __instance.Chat.gameObject.SetActive(true);

                    object aspectPosition = AeroMenuGUI.GetHudAspectPosition(__instance);
                    if (aspectPosition != null)
                    {
                        float camSize = Camera.main != null ? Camera.main.orthographicSize : 3f;
                        if ((!AeroMenuGUI.hudZoomBaseCaptured || (!AeroMenuGUI.zoomOwnsCamera && camSize <= 3.05f)) &&
                            AeroMenuGUI.TryGetAspectDistance(aspectPosition, out Vector3 currentDistance))
                        {
                            AeroMenuGUI.hudZoomBaseDistance = currentDistance;
                            AeroMenuGUI.hudZoomBaseCaptured = true;
                        }

                        if (!hudModalActive && AeroMenuGUI.zoomOwnsCamera && AeroMenuGUI.hudZoomBaseCaptured)
                        {
                            Vector3 distance = AeroMenuGUI.hudZoomBaseDistance;
                            distance.y = AeroMenuGUI.hudZoomBaseDistance.y + 3f * (camSize - 3f);
                            AeroMenuGUI.TrySetAspectDistance(aspectPosition, distance);
                        }
                        else if (hudModalActive && AeroMenuGUI.hudZoomBaseCaptured)
                        {
                            AeroMenuGUI.TrySetAspectDistance(aspectPosition, AeroMenuGUI.hudZoomBaseDistance);
                        }
                    }

                    if (!hudModalActive && AeroMenuGUI.zoomOwnsCamera && __instance.TaskPanel != null && __instance.TaskPanel.gameObject != null && ShipStatus.Instance != null && MeetingHud.Instance == null)
...
[HarmonyPatch(typeof(FullAccount), nameof(FullAccount.CanSetCustomName))]
        public static class FullAccount_CanSetCustomName_Patch { public static void Prefix(ref bool canSetName) { try { if (AeroMenuGUI.unlockFeatures || AeroMenuGUI.guestExtraFeatures) canSetName = true; } catch { } } }

[HarmonyPatch(typeof(AccountManager), nameof(AccountManager.CanPlayOnline))]
        public static class AccountManager_CanPlayOnline_Patch { public static void Postfix(ref bool __result) { try { if (AeroMenuGUI.unlockFeatures || AeroMenuGUI.bypassAgeRestrictions) __result = true; } catch { } } }

[HarmonyPatch(typeof(EOSManager), "IsFreechatAllowed")]
        public static class EOSManager_IsFreechatAllowed_Patch { public static void Postfix(ref bool __result) { try { if (AeroMenuGUI.guestExtraFeatures) __result = true; } catch { } } }

[HarmonyPatch(typeof(EOSManager), "IsFriendsListAllowed")]
        public static class EOSManager_IsFriendsListAllowed_Patch { public static void Postfix(ref bool __result) { try { if (AeroMenuGUI.guestExtraFeatures) __result = true; } catch { } } }

[HarmonyPatch(typeof(EOSManager), "IsMinorOrWaiting")]
        public static class EOSManager_IsMinorOrWaiting_Patch { public static void Postfix(ref bool __result) { try { if (AeroMenuGUI.bypassAgeRestrictions) __result = false; } catch { } } }

[HarmonyPatch(typeof(EOSManager), "IsAllowedOnline")]
        public static class EOSManager_IsAllowedOnline_Patch { public static void Prefix(ref bool canOnline) { try { if (AeroMenuGUI.bypassAgeRestrictions) canOnline = true; } catch { } } }
...
[HarmonyPatch(typeof(InnerNetClient), "JoinGame")]
        public static class InnerNetClient_JoinGame_AccountReady_Patch
        {
            public static void Prefix()
            {
                try
                {
                    if (!AeroMenuGUI.bypassAgeRestrictions) return;
...
[HarmonyPatch(typeof(EngineerRole), "FixedUpdate")]
        public static class EngineerCheatsPatch
        {
            public static void Postfix(EngineerRole __instance)
            {
                ...
                if (AeroMenuGUI.endlessVentTime) __instance.inVentTimeRemaining = float.MaxValue;
                if ((AeroMenuGUI.noAbilityCooldown || AeroMenuGUI.noVentCooldown) && __instance.cooldownSecondsRemaining > 0f)
                {
...
[HarmonyPatch(typeof(TrackerRole), "FixedUpdate")]
        public static class TrackerNoAbilityCooldownPatch
        {
            ...
            {
                if (!AeroMenuGUI.noAbilityCooldown || __instance.Player != PlayerControl.LocalPlayer) return;
...
[HarmonyPatch(typeof(Vent), nameof(Vent.CanUse))]
        public static class Vent_CanUse_UnlockVents_Patch
        {
            public static void Postfix(Vent __instance, NetworkedPlayerInfo pc, ref bool canUse, ref bool couldUse, ref float __result)
            {
                try
                {
                    if (!AeroMenuGUI.unlockVents || __instance == null || pc == null || pc.Object == null)
                        return;
...
[HarmonyPatch(typeof(Ladder), "SetDestinationCooldown")]
        public static class Ladder_SetDestinationCooldown_Patch
        {
            public static bool Prefix(Ladder __instance)
            {
                try
                {
                    if (!AeroMenuGUI.noMapCooldowns) return true;
...
[HarmonyPatch(typeof(ZiplineConsole), "Update")]
        public static class ZiplineConsole_Update_Patch
        {
            public static void Postfix(ZiplineConsole __instance)
            {
                try
                {
                    if (!AeroMenuGUI.noMapCooldowns) return;
                    TrySetCooldownMember(__instance, 0f);
                }
                catch { }
            }
        ...
        public static class KillCooldownTrackerPatch2
        {
            public static void Prefix(PlayerControl __instance, PlayerControl target)
            {
                try
                {
                    ...
                    if (!AeroMenuGUI.spamReportBodies) return;
                    ...
                    AeroMenuGUI.TryOpenModdedMeeting(PlayerControl.LocalPlayer, target.Data);
                }
                catch { }
            }
        }
...
[HarmonyPatch(typeof(ScientistRole), "Update")]
        public static class ScientistCheatsPatch
        {
            ...
            {
                if (__instance.Player != PlayerControl.LocalPlayer) return;
                if (AeroMenuGUI.noAbilityCooldown || AeroMenuGUI.noVitalsCooldown) __instance.currentCooldown = 0f;
                if (AeroMenuGUI.endlessBattery) __instance.currentCharge = float.MaxValue;
            }
        }
...
[HarmonyPatch(typeof(ShapeshifterRole), "FixedUpdate")]
        public static class ShapeshifterDurationPatch
        {
            ...
            {
                if (__instance.Player != PlayerControl.LocalPlayer) return;
                if (AeroMenuGUI.endlessSsDuration) __instance.durationSecondsRemaining = float.MaxValue;
                if (!AeroMenuGUI.noAbilityCooldown || __instance.cooldownSecondsRemaining <= 0f) return;
                __instance.cooldownSecondsRemaining = 0f;
                var btn = DestroyableSingleton<HudManager>.Instance?.AbilityButton;
                if (btn != null) { btn.ResetCoolDown(); btn.SetCooldownFill(0f); }
            }
...
[HarmonyPatch(typeof(ImpostorRole), "FindClosestTarget")]
        public static class ImpostorRangePatch
        {
            ...
            {
                if (!AeroMenuGUI.killReach) return true;
                try
                {
                    var target = AeroMenuGUI.FindClosestKillTarget(__instance, float.MaxValue);
...
[HarmonyPatch(typeof(ImpostorRole), "FindClosestTarget")]
        public static class ImpostorKillAnyoneTargetPatch
        {
            public static void Postfix(ImpostorRole __instance, ref PlayerControl __result)
            {
                try
                {
                    if (!AeroMenuGUI.killAnyone || AeroMenuGUI.killReach) return;
                    __result = AeroMenuGUI.FindClosestKillTarget(__instance, AeroMenuGUI.GetVanillaKillDistance());
                }
                catch { }
            }
        }
        ...
        public static class ImpostorKillWhileVanishedPatch
        {
            public static void Postfix(ImpostorRole __instance, ref PlayerControl __result)
            {
                try
                {
                    ...
                    if (!AeroMenuGUI.killWhileVanishedHostOnly) return;
                    if (AmongUsClient.Instance == null || !AmongUsClient.Instance.AmHost) return;
                    if (!AeroMenuGUI.IsLocalPhantomVanished()) return;
        ...
        public static class PhantomRole_IsValidTarget_KillWhileVanished_Patch
        {
            public static void Postfix(NetworkedPlayerInfo target, ref bool __result)
            {
                try
                {
                    ...
                    __result = AeroMenuGUI.IsAeroValidKillTarget(target);
                }
                catch { }
            }
        }
...
[HarmonyPatch(typeof(PlayerControl), nameof(PlayerControl.CmdCheckMurder))]
        public static class PlayerControl_CmdCheckMurder_KillWhileVanished_Patch
        {
            public static bool Prefix(PlayerControl __instance, PlayerControl target)
            {
                try
                {
                    if (!AeroMenuGUI.killWhileVanishedHostOnly) return true;
                    ...
                    if (!AeroMenuGUI.IsLocalPhantomVanished()) return true;
                    if (target == null || target.Data == null || !AeroMenuGUI.IsAeroValidKillTarget(target.Data)) return true;
        ...
        public static class PlayerControl_CheckMurder_GlitchRoomProtection_Patch
        {
            ...
            private static void ProtectTarget(PlayerControl target)
            {
                try
                {
                    ...
                    bool godMode = AeroMenuGUI.glitchRoomGodModeAll ||
                                   (AeroMenuGUI.glitchRoomGodMode && target == PlayerControl.LocalPlayer);
                    if (!godMode && !AeroMenuGUI.IsGlitchRoomProtected(target)) return;
...
[HarmonyPatch(typeof(PlayerControl), nameof(PlayerControl.CmdCheckMurder))]
        public static class PlayerControl_CmdCheckMurder_KillAnyone_Patch
        {
            public static bool Prefix(PlayerControl __instance, PlayerControl target)
            {
                try
                {
                    if (AeroMenuGUI.glitchRoomBypassShield &&
                        AmongUsClient.Instance != null &&
                        AmongUsClient.Instance.AmHost &&
                        __instance == PlayerControl.LocalPlayer &&
                        target != null &&
                        ...
                        target.Data != null &&
                        AeroMenuGUI.IsAeroValidKillTarget(target.Data))
                    {
                        __instance.RpcMurderPlayer(target, true);
                        return false;
                    }

                    if (!AeroMenuGUI.killAnyone || __instance != PlayerControl.LocalPlayer) return true;
                    if (target == null || target.Data == null || !AeroMenuGUI.IsAeroValidKillTarget(target.Data)) return true;
                    if (AmongUsClient.Instance == null || !AmongUsClient.Instance.AmHost) return true;
                    if (AeroMenuGUI.killWhileVanishedHostOnly && AeroMenuGUI.IsLocalPhantomVanished()) return true;
...
[HarmonyPatch(typeof(ImpostorRole), "IsValidTarget")]
        ...
        {
            public static void Postfix(NetworkedPlayerInfo target, ref bool __result) { try { if (AeroMenuGUI.killAnyone) __result = AeroMenuGUI.IsAeroValidKillTarget(target); } catch { } }
        }
...
[HarmonyPatch(typeof(DetectiveRole), "FindClosestTarget")]
        public static class DetectiveRangePatch
        {
            ...
            {
                if (!AeroMenuGUI.UnlimitedInterrogateRange) return true;
                try
                {
...
[HarmonyPatch(typeof(DoorBreakerGame), nameof(DoorBreakerGame.Start))]
        public static class DoorBreakerGame_Start_Patch
        {
            ...
            {
                if (!AeroMenuGUI.autoOpenDoors) return true;
...
[HarmonyPatch(typeof(MushroomDoorSabotageMinigame), nameof(MushroomDoorSabotageMinigame.Begin))]
        ...
        {
            public static bool Prefix(MushroomDoorSabotageMinigame __instance) { if (AeroMenuGUI.autoOpenDoors) { __instance.FixDoorAndCloseMinigame(); return false; } return true; }
        }
        ...
        public static class Console_CanUse_AllowTasksAsImpostor_Patch
        {
            ...
            public static void Prefix(global::Console __instance, NetworkedPlayerInfo pc)
            {
                try
                {
                    ...
                    bool isImpostor = AeroMenuGUI.IsLocalImpostorRole(pc);
                    if (AeroMenuGUI.allowTasksAsImpostor || !isImpostor)
                    {
            ...
            public static void Postfix(global::Console __instance, NetworkedPlayerInfo pc, ref bool canUse, ref bool couldUse, ref float __result)
            {
                try
                {
                    if (!AeroMenuGUI.allowTasksAsImpostor) return;
                    if (__instance == null || pc == null || pc.Object != PlayerControl.LocalPlayer) return;
                    if (!AeroMenuGUI.IsLocalImpostorRole(pc)) return;

                    __instance.AllowImpostor = true;

                    if (!AeroMenuGUI.LocalPlayerHasTaskForConsole(__instance))
                    {
                        canUse = false;
                        couldUse = false;
                        return;
                    ...
                    if (couldUse)
                        __result = Mathf.Min(__result, AeroMenuGUI.GetConsoleUsableDistance(__instance));
                }
                catch { }
            }
        }
...
[HarmonyPatch]
        public static class Console_Use_AllowTasksAsImpostor_Guard_Patch
        {
            ...
            public static bool Prefix(object __instance)
            {
                try
                {
                    if (__instance is global::Console console && AeroMenuGUI.ShouldBlockUnsafeConsoleUse(console))
                        return false;
                }
                catch { }
...
[HarmonyPatch(typeof(VentilationSystem), nameof(VentilationSystem.Update))]
        public static class VentilationSystem_Update_Immortality_Patch
        {
            public static bool Prefix(VentilationSystem.Operation op, int ventId)
            {
                try
                {
                    if (ventId != AeroMenuGUI.ImmortalityCustomVentId &&
                        AeroMenuGUI.roleBuffImmortality &&
                        AeroMenuGUI.immortalityVentStateApplied &&
...
[HarmonyPatch(typeof(GameManager), nameof(GameManager.StartGame))]
        public static class GameManager_StartGame_Immortality_Patch
        {
            public static void Postfix()
            {
                if (AeroMenuGUI.roleBuffImmortality)
                    AeroMenuGUI.SetImmortalityVentState(true);
            }
        }
...
[HarmonyPatch(typeof(PlayerControl), nameof(PlayerControl.MurderPlayer))]
        public static class PlayerControl_MurderPlayer_ImmortalityNotice_Patch
        {
            public static void Postfix(PlayerControl __instance, PlayerControl target)
            {
                try
                {
                    if (!AeroMenuGUI.roleBuffImmortality || target != PlayerControl.LocalPlayer || __instance == null || __instance.Data == null) return;
