...
private static bool IsAeroValidKillTarget(NetworkedPlayerInfo target)
        {
            try
            {
...
public static bool TryHostAeroMurderPlayer(PlayerControl target)
        {
            try
            {
...
private static bool TelekillPlayerPPM(PlayerControl target)
        {
            try
            {
                ...
                if (AmongUsClient.Instance != null && AmongUsClient.Instance.AmHost)
                    ok = TryHostAeroMurderPlayer(target);
                else
                {
