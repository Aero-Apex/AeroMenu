...
public static class RevealVotesPatch
{
    ...
    internal static void DrawVote(MeetingHud meeting, byte voterId, byte votedForId)
    {
        ...
        {
            if (state == null || (byte)state.PlayerId != votedForId) continue;
    ...
    {
        if (!AeroMenuGUI.RevealVotesEnabled) return;
        try
        {
            ...
            {
                if (item == null) continue;
                var playerById = GameData.Instance.GetPlayerById((byte)item.PlayerId);
                if (playerById == null || playerById.Disconnected || _votedPlayers.Contains((byte)item.PlayerId)) continue;
                byte votedForId = _voteTargets.TryGetValue((byte)item.PlayerId, out byte rememberedVote) ? rememberedVote : (byte)item.VotedForId;
                DrawVote(__instance, (byte)item.PlayerId, votedForId);
            }
...
public static class RevealVotesCastVotePatch
{
    ...
    {
        RevealVotesPatch.RememberVote((byte)srcPlayerId, (byte)suspectPlayerId);
    }
}
