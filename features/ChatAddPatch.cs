...
public static class ChatController_AddChat_Patch
{
    ...
    public static bool Prefix(PlayerControl sourcePlayer, ref string chatText, bool censor, ChatController __instance)
    {
        ...
        AeroMenuGUI.AddPortableChatLog(sourcePlayer, chatText);
        string lowerText = chatText.ToLower().Trim();

        if (AeroMenuGUI.enableColorCommand && sourcePlayer != null)
        {
            ...
            if (usedCmd != null)
            {
                if (AmongUsClient.Instance != null && AmongUsClient.Instance.AmHost)
                {
                    ...
                    else colorId = AeroMenuGUI.GetColorIdByName(colorInput);

                    if (colorId != -1)
                    {
                        if (colorId == 18 && AeroMenuGUI.blockFortegreenChat)
                        {
            ...
            if (lowerText == "/rainbow" || lowerText == "!rainbow" || lowerText == "/lgbt" || lowerText == "!lgbt")
            {
                if (AmongUsClient.Instance != null && AmongUsClient.Instance.AmHost)
                {
                    if (AeroMenuGUI.blockRainbowChat)
                    {
                        if (HudManager.Instance?.Chat != null)
                    ...
                    {
                        if (AeroMenuGUI.rainbowPlayers.Contains(sourcePlayer.PlayerId))
                        {
                            AeroMenuGUI.rainbowPlayers.Remove(sourcePlayer.PlayerId);
                            AeroMenuGUI.ShowNotification("<color=#FF00FF>[SERVER]</color> Rainbow OFF.");
                        }
                        else
                        {
                            AeroMenuGUI.rainbowPlayers.Add(sourcePlayer.PlayerId);
                            AeroMenuGUI.ShowNotification("<color=#FF00FF>[SERVER]</color> Rainbow ON.");
                        }
                    }
                }
                return false;
        ...
        if (AeroMenuGUI.showChatRoles && sourcePlayer != null && sourcePlayer.Data != null && sourcePlayer.Data.Role != null)
        {
            if (!chatText.StartsWith("<color=#"))
            {
                string roleTag = AeroMenuGUI.GetChatRoleTag(sourcePlayer.Data);
                if (!string.IsNullOrEmpty(roleTag)) chatText = roleTag + chatText;
            }
        }
    ...
    private static bool ShouldShowGhostMessage(PlayerControl sourcePlayer)
    {
        try
        {
            if (!AeroMenuGUI.readGhostChat && !AeroMenuGUI.seeGhosts) return false;
    ...
    private static bool ShowGhostMessage(PlayerControl sourcePlayer, string chatText, bool censor, ChatController chat)
    {
        ...
        try
        {
            ...
            pooledBubble.SetText(AeroMenuGUI.RenderGhostChatMessageText(chatText));
