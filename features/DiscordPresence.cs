...
public sealed class AeroDiscordPresence : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (!AeroMenuGUI.discordRpcEnabled)
        {
    ...
    private void Push()
    {
        string details = "AeroMenu · " + Plugin.DisplayVersion;
        string state = Scene();
    ...
    private static string Scene()
    {
        try
        {
            ...
            if (MeetingHud.Instance != null) return AeroMenuGUI.L("Meeting", "Собрание") + tail;
            if (ShipStatus.Instance != null) return AeroMenuGUI.L("In game", "В игре") + tail;
            if (LobbyBehaviour.Instance != null) return AeroMenuGUI.L("Lobby", "Лобби") + tail;
        }
        catch { }
        return AeroMenuGUI.L("In menu", "В меню");
    }
...
internal static class AeroDiscordSilence
{
