...
{
    private const string CenteredHostNameObject = "AeroCenteredLobbyHostName";
    ...
    public static void UpdateStyledNames(bool force = false)
    {
        ...
        for (int i = StyledNames.Count - 1; i >= 0; i--)
        {
            try
            {
                ...
                entry.Text.gameObject.SetActive(AeroMenuGUI.moreLobbyInfo);
                if (!AeroMenuGUI.moreLobbyInfo)
                {
                    entry.InfoText.text = entry.OriginalInfoText;
                    continue;
                }

                string styledName = AeroMenuGUI.rgbMenuMode
                    ? $"<color=#{AeroMenuGUI.GetMenuControlAccentHex()}>{entry.HostName}</color>"
                    : AeroMenuGUI.ApplyMenuShimmer(entry.HostName);
    ...
    {
        return AeroMenuGUI.rgbMenuMode
            ? $"<color=#{AeroMenuGUI.GetMenuControlAccentHex()}>{value}</color>"
            : AeroMenuGUI.ApplyMenuShimmer(value);
    }
