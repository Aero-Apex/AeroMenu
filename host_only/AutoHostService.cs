...
namespace AeroMenu
{
    ...
    {
public static class AeroAutoLobbyReturn
        {
            ...
            {
                return AeroMenuGUI.AutoReturnLobbyAfterMatch || AeroAutoHostService.ShouldReturnAfterMatch;
            }
...
public static class AeroAutoHostService
        {
            ...
            public static AutoHostStatusSnapshot GetStatusSnapshot()
            {
                AutoHostStatusSnapshot snapshot = new AutoHostStatusSnapshot
                {
                    ...
                    StartMode = AeroMenuGUI.AutoHostInstantStart ? "Мгновенный" : "Обычный",
                    ...
                    ShieldBreakEnabled = AeroMenuGUI.AutoHostShieldBreakEnabled,
            ...
            public static string TryStartNow()
            {
                ...
                if (AeroMenuGUI.AutoHostWaitLoadedPlayers && !AeroMenuGUI.AreAllLobbyPlayersLoadedForStart(out int connectedPlayers, out int readyPlayers, out string loadingName))
                {
            ...
            private static void TickHostedLobby(InnerNetClient client, float now)
            {
                ...
                bool waitingForLoad = !autoRun && AeroMenuGUI.AutoHostWaitLoadedPlayers && connectedPlayers > readyPlayers;
                if (waitingForLoad)
                {
                ...
                bool enoughPlayers = autoRun
                    ? connectedPlayers >= 1
                    : AeroMenuGUI.AutoHostWaitLoadedPlayers ? readyPlayers >= requiredPlayers : connectedPlayers >= requiredPlayers;
                bool continueBelowMin = !AeroMenuGUI.AutoHostCancelBelowMin && countdownStartedAt >= 0f && connectedPlayers >= 2;
            ...
            private static bool TryConfiguredStart(GameStartManager manager)
            {
                ...
                try
                {
                    ...
                    if (AeroMenuGUI.AutoHostInstantStart || AutoRunEnabled)
                    {
            ...
            private static void TickAutoRunEnd(float now)
            {
                ...
                if (!AeroMenuGUI.HasCurrentGameSeenShhh())
                {
            ...
            private static bool TryEndGameAsImpostorWin()
            {
                ...
                try
                {
                    if (!AeroMenuGUI.CanRunHostEndGameAction(false) || GameManager.Instance == null)
                        return false;

                    int reasonCode = GameManager.Instance.IsHideAndSeek() ? 8 : 3;
                    tempBlock = AeroMenuGUI.neverEndGame;
                    changedBlock = true;
                    AeroMenuGUI.neverEndGame = false;
                ...
                {
                    if (changedBlock) AeroMenuGUI.neverEndGame = tempBlock;
                }
            }

            private static void TickAutoShieldBreak(float now)
            {
                if (!AeroMenuGUI.AutoHostShieldBreakEnabled || !IsEnabled)
                {
            ...
            private static bool CanBypassLoadWait(float now, int readyPlayers, int connectedPlayers, string loadingName)
            {
                ...
                int grace = Mathf.Clamp((int)AeroMenuGUI.AutoHostLoadGraceSeconds, 0, 90);
            ...
            private static bool ShouldForceStart(int connectedPlayers, out string reason)
            {
                ...
                int forceAfterMinutes = Mathf.Clamp(AeroMenuGUI.AutoHostForceAfterMinutes, 0, 10);
            ...
            {
                int threshold = Mathf.Clamp(AeroMenuGUI.AutoHostFastStartPlayers, 0, 15);
            ...
            private static float EffectiveStartDelay(int connectedPlayers)
            {
                ...
                if (IsFastStartActive(connectedPlayers))
                    delay = Mathf.Min(delay, Mathf.Clamp(AeroMenuGUI.AutoHostFastStartDelaySeconds, 0, 60));
                return delay;
            }
            ...
            {
                if (!AeroMenuGUI.AutoHostNotifications) return;
                float now = Time.unscaledTime;
                if (lastNotificationAt > 0f && now - lastNotificationAt < NotificationCooldownSeconds) return;
                lastNotificationAt = now;
                AeroMenuGUI.ShowNotification($"<color=#FF00FF>[{title}]</color> {detail}");
            }
            ...
            public static bool IsEnabled => AeroMenuGUI.AutoHostEnabled || AutoRunEnabled;
            private static bool AutoRunEnabled => AeroMenuGUI.AutoHostAutoRunEnabled;
            public static bool ShouldReturnAfterMatch => IsEnabled && AeroMenuGUI.AutoReturnLobbyAfterMatch;
            private static bool ForceLastMinuteEnabled => AeroMenuGUI.AutoHostForceLastMinute;
            private static int RequiredPlayers => Mathf.Clamp(AeroMenuGUI.AutoHostMinPlayers, 1, 15);
            private static int ForceMinPlayers => Mathf.Clamp(AeroMenuGUI.AutoHostForceMinPlayers, 1, 15);
            private static float StartDelaySeconds => Mathf.Clamp(AeroMenuGUI.AutoHostStartDelaySeconds, 0f, 180f);
            private static float AutoRunStartDelaySeconds => Mathf.Clamp(AeroMenuGUI.AutoHostAutoRunDelaySeconds, 0.25f, 10f);
            private static float BackoffSeconds => Mathf.Clamp(AeroMenuGUI.AutoHostBackoffSeconds, 2f, 60f);
            ...
            private static float WarmupRemaining => lobbyOpenedAt < 0f ? 0f : Mathf.Clamp(AeroMenuGUI.AutoHostWarmupSeconds - (Time.unscaledTime - lobbyOpenedAt), 0f, 120f);
            private static float LoadGraceRemaining => loadWaitStartedAt < 0f || AeroMenuGUI.AutoHostLoadGraceSeconds <= 0 ? 0f : Mathf.Clamp(AeroMenuGUI.AutoHostLoadGraceSeconds - (Time.unscaledTime - loadWaitStartedAt), 0f, 90f);
        }
