...
namespace AeroMenu
{
    public static class AeroBugroomScoutService
    {
        ...
        public static string FilePath => Path.Combine(Plugin.AeroFolder, FileName);

        public static void Tick()
        {
            if (!AeroMenuGUI.BugroomScoutEnabled)
            {
                ResetRoomState();
            ...
            currentCode = AeroMenuGUI.GetCurrentRoomCodeForStatus();
            ...
            if (targetSuffixes.Contains(currentSuffix))
            {
                ...
                exitQueuedAt = -1f;
                AeroMenuGUI.BugroomScoutEnabled = false;
                AeroMenuGUI.settingsDirty = true;
                state = $"Found {currentSuffix}";
                AeroMenuGUI.ShowNotification($"<color=#00FFAA>[BUGROOM SCOUT]</color> Found code: <b>{currentCode}</b>");
                return;
            }
            ...
            if (checkedGameId != gameId)
            {
                ...
                AeroMenuGUI.ShowNotification($"<color=#FFAA00>[BUGROOM SCOUT]</color> {currentCode} != target, leaving.");
            }

            if (queuedExitGameId == gameId && exitQueuedAt >= 0f && Time.unscaledTime >= exitQueuedAt)
            {
                string latestCode = AeroMenuGUI.GetCurrentRoomCodeForStatus();
                ...
                if (!string.IsNullOrEmpty(latestSuffix) && targetSuffixes.Contains(latestSuffix))
                {
                    ...
                    AeroMenuGUI.ShowNotification($"<color=#00FFAA>[BUGROOM SCOUT]</color> Found code: <b>{latestCode}</b>");
                    return;
                }
        ...
        public static BugroomScoutStatusSnapshot GetStatusSnapshot()
        {
            ...
            {
                Enabled = AeroMenuGUI.BugroomScoutEnabled,
        ...
        public static void ForceReload()
        {
            ...
            if (!AeroMenuGUI.BugroomScoutEnabled) state = "Off";
            EnsureFile();
            ReloadTargetsIfNeeded();
        }
        ...
        private static void EnsureFile()
        {
            try
            {
                Directory.CreateDirectory(Plugin.AeroFolder);
    ...
    public static class AeroBugroomFarmService
    {
        ...
        public static BugroomFarmStatusSnapshot GetStatusSnapshot()
        {
            ...
            {
                MainEnabled = AeroMenuGUI.bugRoomLv35Rehost,
                PassEnabled = AeroMenuGUI.bugRoomHostPassRejoin,
                RoundEnabled = AeroMenuGUI.bugRoomAutoRejoin,
        ...
        public static void ResetMain()
        {
            ...
            mainLeaving = false;
            mainJoining = false;
            mainState = AeroMenuGUI.bugRoomLv35Rehost ? "Idle" : "Off";
        }
        ...
        public static void ResetPass()
        {
            ...
            passLeaving = false;
            passJoining = false;
            passState = AeroMenuGUI.bugRoomHostPassRejoin ? "Waiting host" : "Off";
        }
        ...
        private static void ResetRound()
        {
            ...
            roundLeaving = false;
            roundJoining = false;
            roundState = AeroMenuGUI.bugRoomAutoRejoin ? "Waiting game" : "Off";
        }

        private static bool TickRound()
        {
            if (!AeroMenuGUI.bugRoomAutoRejoin)
            {
                ResetRound();
                return false;
            }
            ...
            if (roundJoining)
            {
                ...
                if (now >= roundAt)
                {
                    ...
                    AeroMenuGUI.ShowNotification("<color=#FF4444>[BUG ROOM]</color> Rejoin failed, code copied.");
                }
                return true;
            }
            ...
            roundLeaving = true;
            roundState = "Leaving";
            AeroMenuGUI.ShowNotification("<color=#FF00FF>[BUG ROOM]</color> Rejoining after game.");
            return true;
        }

        private static void TickMain()
        {
            if (!AeroMenuGUI.bugRoomLv35Rehost)
            {
                ResetMain();
                return;
            }
            ...
            if (mainJoining)
            {
                if (InRoom())
                {
                    ...
                    mainSawGame = false;
                    mainAt = -1f;
                    AeroMenuGUI.AutoHostAutoRunEnabled = false;
                ...
                if (now >= mainAt)
                {
                    ...
                    AeroMenuGUI.bugRoomLv35Rehost = false;
                    AeroMenuGUI.settingsDirty = true;
                    ResetMain();
                    AeroMenuGUI.ShowNotification("<color=#FF4444>[BUG ROOM]</color> Main rejoin failed, code copied.");
                }
                return;
            }
            ...
            if (mainRun && mainSawGame && LobbyBehaviour.Instance != null)
            {
                ...
                if (lvl >= 35)
                {
                    ...
                    AeroMenuGUI.ShowNotification("<color=#FF00FF>[BUG ROOM]</color> Lv35 reached, rejoining room.");
                    return;
                }
            ...
            if (AeroMenuGUI.AutoHostMinPlayers > 2)
                AeroMenuGUI.AutoHostMinPlayers = 2;
            AeroMenuGUI.AutoReturnLobbyAfterMatch = true;
            AeroMenuGUI.AutoHostAutoRunEnabled = true;
            AeroMenuGUI.settingsDirty = true;
            mainRun = true;
            mainSawGame = false;
            mainState = "Auto Run ON";
            AeroMenuGUI.ShowNotification("<color=#FF00FF>[BUG ROOM]</color> Auto Run enabled after 4m.");
        }

        private static void TickPass()
        {
            if (!AeroMenuGUI.bugRoomHostPassRejoin)
            {
                ResetPass();
                return;
            }
            ...
            if (passJoining)
            {
                ...
                if (now >= passAt)
                {
                    ...
                    ResetPass();
                    AeroMenuGUI.ShowNotification("<color=#FF4444>[BUG ROOM]</color> Pass rejoin failed, code copied.");
                }
                return;
            }
            ...
            passLeaving = true;
            passState = "Leaving";
            AeroMenuGUI.ShowNotification("<color=#FF00FF>[BUG ROOM]</color> Host pass rejoin.");
        }
