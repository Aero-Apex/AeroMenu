...
public static bool isWaitBindToggleAutoFix = false;
        public static bool isWaitBindCopyCode = false;
...
private static AeroMenuGUI activeGui;
...
private string[] menuColorNames = {
            "Aero Blue", "Dark Forest", "Green", "Sea Green", "Mint", "Chartreuse",
...
private static readonly string[] tabNames = { L("GENERAL", "ОБЩИЕ"), L("SELF", "ИГРОК"), L("VISUALS", "ВИЗУАЛ"), L("PLAYERS", "ИГРОКИ"), L("SABOTAGES", "САБОТАЖИ"), L("HOST ONLY", "ХОСТ"), L("VOTEKICK", "КИК"), L("MENU", "МЕНЮ"), L("SEARCH", "ПОИСК") }
...
[HarmonyPatch(typeof(TextBoxTMP), nameof(TextBoxTMP.Start))]
        public static class AllowSymbols_TextBoxTMP_Start_Patch
        {
            ...
            {
                __instance.allowAllCharacters = AeroMenuGUI.allowLinksAndSymbols;
                __instance.AllowSymbols = AeroMenuGUI.allowLinksAndSymbols;
                __instance.AllowEmail = AeroMenuGUI.allowLinksAndSymbols;
            }
        }
...
[HarmonyPatch(typeof(ChatController), nameof(ChatController.Update))]
        public static class ChatJailbreak_ChatController_Update_Postfix
        {
            public static void Postfix(ChatController __instance)
            {
                ...
                if (AeroMenuGUI.enableFastChat && __instance.timeSinceLastMessage < 0.9f)
                {
                    __instance.timeSinceLastMessage = 0.9f;
                }

                __instance.freeChatField.textArea.allowAllCharacters = AeroMenuGUI.allowLinksAndSymbols;
                __instance.freeChatField.textArea.AllowSymbols = AeroMenuGUI.allowLinksAndSymbols;
                __instance.freeChatField.textArea.AllowEmail = AeroMenuGUI.allowLinksAndSymbols;

                __instance.freeChatField.textArea.characterLimit = AeroMenuGUI.enableExtendedChat ? 120 : 100;
            }
        }
...
[HarmonyPatch(typeof(ChatController), nameof(ChatController.SendFreeChat))]
        public static class AllowURLS_ChatController_SendFreeChat_Patch
        {
            public static bool Prefix(ChatController __instance)
            {
                ...
                if (AeroMenuGUI.enableChatHistory && !string.IsNullOrWhiteSpace(text))
                    AeroMenuGUI.ChatHistory.Remember(text.Trim());

                if (AeroMenuGUI.autoChatEveryone)
                {
                    string msg = CensorUrlsAndEmails(text);
                    if (!string.IsNullOrWhiteSpace(msg))
                    {
                        InjectChatSpawnForAll();
                ...
                if (!AeroMenuGUI.allowLinksAndSymbols) return true;
            ...
            private static string CensorUrlsAndEmails(string text)
            {
                ...
                return regex.Replace(text ?? string.Empty, match => match.Value.Replace('.', ','));
            }

            private static void InjectChatSpawnForAll()
            {
                ...
                try
                {
                    MessageWriter wr = MessageWriter.Get(SendOption.Reliable);
                    wr.StartMessage(6);
                    wr.Write(AmongUsClient.Instance.GameId);
                    wr.WritePacked(-1);
