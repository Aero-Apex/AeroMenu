...
public static class AmongUsClient_KickPlayer_BanList_Patch
{
    public static bool Prefix(InnerNetClient __instance, ref int clientId, bool ban)
    {
        ...
        if (AeroMenuGUI.IsBuiltInProtected(clientId))
            return false;
                ...
                if (pc.Data != null)
                {
                    ...
                    try
                    {
                        ...
                        if (client != null && !string.IsNullOrWhiteSpace(client.ProductUserId))
                            ...
                            puid = AeroMenuGUI.GetPlayerPuid(pc);
                    }
                    catch { }

                    AeroMenuGUI.AddToBanList(fc, puid, name, "Host ban");
                    AeroMenuGUI.ShowNotification($"<color=#FF0000>[BAN]</color> {name} added to ban list!");
                }
            }
            catch { }
        }
