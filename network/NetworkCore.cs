                        ...
                        if (!RpcCrash)
                        {
                            new LateTask(delegate
                            {
                               DestroyableSingleton<HudManager>.Instance.Notifier.AddDisconnectMessage($"<color=#FFFF00>Received Rpc from <b>{player.Data.PlayerName}</b>, {b}\nthat shouldn't be got like that way</color>");
                                if (enablePasosLimit)
                                {
                                    AeroMenuGUI.RegisterAntiCheatDisconnectNotice(player.OwnerId, player.Data.PlayerName, $"Invalid targeted RPC ({b})", true);
                    ...
                    if (AmongUsClient.Instance.GameState == InnerNetClient.GameStates.Joined && !excludedCallIdsForLobby.Contains(callId) && callId < 66 && player != PlayerControl.LocalPlayer)
                    {
                        if (!RpcCrash)
                        {
                            new LateTask(delegate
                            {
                                ...
                                if (enablePasosLimit)
                                {
                                    AeroMenuGUI.RegisterAntiCheatDisconnectNotice(player.OwnerId, player.Data.PlayerName, $"Game RPC in lobby ({b})", true);
                                    AmongUsClient.Instance.KickPlayer(player.OwnerId, true);
                                }
                            }, 2f);
                            _ = new LateTask(delegate
                ...
                if ((queue.Count > num2) && (byte)RpcCalls.SnapTo != callId)
                {
                    if (!RpcCrash)
                    {
                        new LateTask(delegate
                        {
                            if (enablePasosLimit)
                            {
                                AeroMenuGUI.RegisterAntiCheatDisconnectNotice(player.OwnerId, player.Data != null ? player.Data.PlayerName : player.name, $"RPC spam ({(RpcCalls)callId})", true);
                                AmongUsClient.Instance.KickPlayer(player.OwnerId, true);
                            }
                ...
                if (!Crashed)
                {
                    ...
                    _ = new LateTask(delegate
                    {
                        ...
                        if (banMalformedPacketSender)
                        {
                            ...
                            AeroMenuGUI.RegisterAntiCheatDisconnectNotice(keyValuePair.Key, $"Client {keyValuePair.Key}", $"Malformed packet: {reason}", true);
                    ...
                    case GameDataTypes.SceneChangeFlag:
                        {
                            try
                            {
                                ...
                                if (clientData2 != null && !string.IsNullOrWhiteSpace(text))
                                {
                                    ...
                                    if (AeroMenuGUI.detailedLogsEnabled)
                    ...
                    case GameDataTypes.RpcFlag:
                        try
                        {
                            ...
                            while (true)
                            {
                                ...
                                try
                                {
                                    if (AeroMenuGUI.detailedLogsEnabled && PlayerControl.LocalPlayer)
                                    {
