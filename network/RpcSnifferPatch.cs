...
{
    private static readonly HashSet<byte> VanillaRPCs = AeroMenuGUI.VanillaRpcIds;
    ...
    private static Dictionary<byte, (string Name, string Color)> BuildKnownMods()
        {
            ...
            int count = Mathf.Min(AeroMenuGUI.spoofMenuRPCs.Length, AeroMenuGUI.spoofMenuNames.Length);
            for (int i = 0; i < count; i++)
            {
                byte rpc = AeroMenuGUI.spoofMenuRPCs[i];
                if (known.ContainsKey(rpc)) continue;
                known[rpc] = (CleanSpoofMenuName(AeroMenuGUI.spoofMenuNames[i]), ColorForKnownRpc(rpc));
            }
            return known;
        }
    ...
    public static bool Prefix(PlayerControl __instance, byte callId, MessageReader reader)
    {
        ...
        AeroMenuGUI.RecordPlayerRpc(__instance, callId);

        if (AeroMenuGUI.LogAllRPCs)
        {
            ...
            if (!VanillaRPCs.Contains(callId))
            {
                ...
                {
                    AeroMenuGUI.ShowNotification($"<color=#00FFFF>[RPC SPOOF]</color> <b>{pNameSniff}</b>: <b><color={modInfo.Color}>{modInfo.Name}</color></b> <color=#FFFF00>({callId})</color>");
                }
                else
                {
                    AeroMenuGUI.ShowNotification($"<color=#00FFFF>[RPC SPOOF]</color> <b>{pNameSniff}</b> unknown RPC: <color=#FFFF00>{callId}</color>");
                }
            }
        }
        ...
        {
            int clientId = AeroNetGuard.NetworkGuard.ResolveCurrentRpcSenderClientId(__instance, callId);
            if (clientId < 0 && __instance.Data != null) clientId = __instance.Data.ClientId;
            if (clientId < 0) clientId = __instance.OwnerId;

            if (clientId >= 0 && !AeroMenuGUI.IsProtectedFromAnticheat(clientId) &&
                !AeroMenuGUI.IsProtectedFromAnticheat(__instance))
            {
                string rpcName = KnownMods.TryGetValue(callId, out var modInfo) ? modInfo.Name : "Unknown";
                AeroNetGuard.NetworkGuard.ApplyMenuProtection(clientId, "Custom RPC", $"{rpcName} ({callId})");
            }
...
public static class RPCSniffer_RawGameDataPatch
{
    ...
    {
        if (__instance == null || parentReader == null || !AeroMenuGUI.LogAllRPCs)
            return;
            ...
            while (copy.Position < copy.Length)
            {
                ...
                try
                {
                    ...
                    if (!IsPlayerRpcTarget(__instance, netId))
                        AeroMenuGUI.ShowNotification($"<color=#00FFFF>[RAW RPC]</color> {objectLabel}: <b>{rpcLabel}</b> <color=#FFFF00>({callId})</color>");
                }
                finally
                {
                    part.Recycle();
        ...
        try
        {
            if (client.allObjects != null && client.allObjects.AllObjectsFast.TryGetValue(netId, out InnerNetObject obj) && obj != null)
            {
                ...
                if (player != null)
                {
                    ...
                    if (SameRpcName(playerName, obj.name)) return playerName;
    ...
    private static bool SameRpcName(string a, string b)
    {
        string ca = CleanRpcName(a);
        string cb = CleanRpcName(b);
        return ca.Length > 0 && string.Equals(ca, cb, StringComparison.OrdinalIgnoreCase);
    }

    private static string CleanRpcName(string s)
    {
    ...
    private static string ResolveRpcLabel(byte callId)
    {
        try
        {
            for (int i = 0; i < AeroMenuGUI.spoofMenuRPCs.Length && i < AeroMenuGUI.spoofMenuNames.Length; i++)
            {
                if (AeroMenuGUI.spoofMenuRPCs[i] == callId)
                    return Regex.Replace(AeroMenuGUI.spoofMenuNames[i], @"\s*\(\d+\)\s*$", string.Empty).Trim();
            }
        }
        catch { }
