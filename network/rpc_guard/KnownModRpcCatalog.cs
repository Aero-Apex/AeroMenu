...
internal static class KnownModRpcCatalog
{
	...
	private static byte[] BuildRpcIds()
	{
		try
		{
			if (AeroMenu.AeroMenuGUI.spoofMenuRPCs != null)
				return AeroMenu.AeroMenuGUI.spoofMenuRPCs.ToArray();
		}
		catch { }
	...
	private static KnownModRpcRule[] BuildRules()
	{
		try
		{
			string[] names = AeroMenu.AeroMenuGUI.spoofMenuNames;
			byte[] rpcs = AeroMenu.AeroMenuGUI.spoofMenuRPCs;
