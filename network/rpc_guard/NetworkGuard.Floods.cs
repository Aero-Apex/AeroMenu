...
private static bool IsLobbyJoinSyncGrace(PlayerControl player = null, int clientId = -1)
	...
	private static bool ShouldBlockPlayerSemanticRpc(PlayerControl player, byte rpcByte, int clientId, MessageReader reader)
	{
		...
		if (rpcByte == 38 && TryReadLevel(reader, out uint level))
		{
			...
			PlayerLevelCache.Remember(player, level);
		...
		if (rpcByte == 1)
		{
			...
			if (IsImpostor(player) && !AeroMenu.AeroMenuGUI.allowTasksAsImpostor)
			{
		...
		if (CosmeticMutationRpcIds.Contains(rpcByte) && InActiveMatch() && !FakeMapLobbyGuard.Active && !MeetingOrExileActive() && IntroCutscene.Instance == null)
		{
...
private static bool IsPopularSpoofLevel(uint level)
	{
		string raw = GuardOptions.SuspiciousLevelList?.Value ?? string.Empty;
...
private static uint MaxAllowedLevelRpc()
	{
		try
		{
			return (uint)Mathf.Clamp(GuardOptions.MaxAllowedLevelRpc?.Value ?? 10000, 100, 10000);
		}
		catch
		{
			return 10000u;
...
private static bool ClientIdentityReady(ClientData client)
	{
		try
		{
			PlayerLoadInfo info = PlayerLoadState.Evaluate(client);
...
private static bool Enabled()
	{
		return (GuardOptions.NetworkProtection == null || GuardOptions.NetworkProtection.Value)
...
private static bool NetIdOverflowProtectionEnabled()
	{
		return (GuardOptions.NetIdOverflowProtection == null || GuardOptions.NetIdOverflowProtection.Value)
			&& AmongUsClient.Instance != null
			&& AmongUsClient.Instance.AmHost;
	}
...
private static bool EnabledNonHostFloodDrop()
	{
		return GuardOptions.FloodDropNonHost != null
			&& GuardOptions.FloodDropNonHost.Value
			&& AmongUsClient.Instance != null
			&& !AmongUsClient.Instance.AmHost;
	}
...
private static bool BrokenFriendCodeBanEnabled()
	{
		return GuardOptions.BanBrokenFriendCode == null || GuardOptions.BanBrokenFriendCode.Value;
	}

private static bool LevelRpcProtectionEnabled()
	{
		return GuardOptions.LevelRpcProtection == null || GuardOptions.LevelRpcProtection.Value;
	}

private static bool IdenticalNetIdProtectionEnabled()
	{
		return GuardOptions.IdenticalNetIdProtection == null || GuardOptions.IdenticalNetIdProtection.Value;
	}

private static bool ChatSpoofProtectionEnabled()
	{
		return GuardOptions.ChatSpoofProtection == null || GuardOptions.ChatSpoofProtection.Value;
	}

private static string ChatSpoofProtectionAction()
	{
		return GuardOptions.ChatSpoofAction == null ? "Kick" : GuardOptions.NormalizeNetworkProtectionAction(GuardOptions.ChatSpoofAction.Value);
	}

private static bool CosmeticSpoofProtectionEnabled()
	{
		return GuardOptions.CosmeticSpoofProtection == null || GuardOptions.CosmeticSpoofProtection.Value;
	}

private static string CosmeticSpoofProtectionAction()
	{
		return GuardOptions.CosmeticSpoofAction == null ? "Warn" : GuardOptions.NormalizeNetworkProtectionAction(GuardOptions.CosmeticSpoofAction.Value);
	}
...
private static bool TryHandleKnownModRpc(PlayerControl player, byte rpcByte, int clientId, out bool skipOriginal)
	{
		...
		try { AeroMenu.AeroMenuGUI.RecordPlayerRpc(player, rpcByte); } catch { }
...
private static string KnownModRpcAction(int ruleIndex)
	{
		try
		{
			ConfigEntry<string>[] actions = GuardOptions.KnownModRpcActions;
...
private static string ProtectionAction()
	{
		return GuardOptions.NetworkProtectionAction == null ? "Ban" : GuardOptions.NormalizeNetworkProtectionAction(GuardOptions.NetworkProtectionAction.Value);
	}

private static string LevelRpcProtectionAction()
	{
		return GuardOptions.LevelRpcAction == null ? "Ban" : GuardOptions.NormalizeNetworkProtectionAction(GuardOptions.LevelRpcAction.Value);
	}
...
private static void BlockMessage(int clientId, string title, string detail, string actionOverride)
	{
		...
		if (shouldNotify)
		{
			GuardPlugin.Logger?.LogWarning((object)$"Network protection blocked message from client {clientId}: {title}. {detail}");
		}

		if (shouldNotify && action != "Null")
		{
			SecurityNotice.Show(action, ClientName(clientId), title, detail, clientId);
		}
...
private static void BlockNetIdOverflow(int clientId, string detail)
	{
		...
		if (clientId < 0 || AmongUsClient.Instance == null || !AmongUsClient.Instance.AmHost)
		{
			...
			{
				GuardPlugin.Logger?.LogWarning((object)$"Network protection dropped NetId overflow with unreliable sender: {detail}");
				SecurityNotice.Show("Warn", null, title, detail, clientId);
			}
			return;
		}
		...
		if (shouldNotify)
		{
			GuardPlugin.Logger?.LogWarning((object)$"Network protection blocked NetId overflow from client {clientId}: {detail}");
...
private static void BlockRpc(PlayerControl player, int clientId, string title, string detail, string actionOverride)
	{
		...
		if (action != "Null")
		{
			SecurityNotice.Show(action, name, title, detail, clientId);
		}
...
internal static void ReportQuickChatFlood(PlayerControl sender, int ownerId, string detail)
	{
		try
		{
			...
			if (action != "Null")
			{
				...
				SecurityNotice.Show(action, name, "QuickChat flood", detail, ownerId);
			}
...
internal static void ApplyMenuProtection(int clientId, string attackType, string detail)
	{
		switch (AeroMenu.AeroMenuGUI.punishmentMode)
		{
			case 3:
				DisconnectIfHost(clientId, true, attackType, detail);
				return;
...
private static void DisconnectIfHost(int clientId, bool ban, string attackType, string detail, bool addToLocalBanList)
	{
		...
		if (AeroMenu.AeroMenuGUI.IsProtectedFromAnticheat(clientId))
		{
			return;
		}
		...
		try
		{
			if (addToLocalBanList)
			{
				...
				if (!BanAccessList.AddBanClient(target, $"{attackType}: {detail}") && TryGetSnapshotIdentity(clientId, out ClientIdentity identity))
				{
					BanAccessList.AddBanIdentity(identity, $"{attackType}: {detail}");
				}
			}

			string reason = string.IsNullOrWhiteSpace(detail) ? attackType : $"{attackType}: {detail}";
			AeroMenu.AeroMenuGUI.RegisterAntiCheatDisconnectNotice(clientId, ClientName(clientId), reason, ban);
			client.KickPlayer(clientId, ban);
			GuardPlugin.Logger?.LogWarning((object)$"Network protection sent {(ban ? "ban" : "kick")} for client {clientId}.");
			QueueLocalCleanup(clientId, 0f);
		}
		catch (Exception error)
		{
			GuardPlugin.Logger?.LogWarning((object)$"Network protection {(ban ? "ban" : "kick")} failed for client {clientId}: {error.Message}");
...
private static bool ForceLocalClientLeave(int clientId)
	{
		...
		if (!stillListed)
		{
			GuardPlugin.Logger?.LogWarning((object)$"Network protection cleaned removed client {clientId} from lobby.");
		}
...
private static void RunNormalPlayerLeft(ClientData client)
	{
		...
		{
			GuardPlugin.Logger?.LogWarning((object)$"Network protection OnPlayerLeft cleanup failed for client {client.Id}: {error.Message}");
		}
	}
...
private static void RemoveClientFromActiveList(InnerNetClient client, ClientData target, int clientId)
	{
		...
		{
			GuardPlugin.Logger?.LogWarning((object)$"Network protection active client cleanup failed for {clientId}: {error.Message}");
		}
	}
