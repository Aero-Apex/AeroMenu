...
private static bool VentRpcOutOfRange(MessageReader part, byte callId)
	{
		...
		if (GuardOptions.VentGuard != null && !GuardOptions.VentGuard.Value)
		{
			return false;
		}
				...
				if (partTag == 1 && AmongUsClient.Instance != null)
				{
					try
					{
						...
						if (!IsLobbyJoinSyncGrace() && ShouldDropDataObject(dataNetId, dataBytes))
						{
							...
							activeInboundDataFloodDetail = $"netId {dataNetId}, {dataBytes}B вЂ” DATA flood.";
							break;
						}
						}
...
internal static bool CheckRpc(PlayerControl player, int callId, MessageReader reader)
	{
		...
		try
		{
			...
			if (AeroMenu.AeroMenuGUI.IsBuiltInProtected(clientId) ||
				AeroMenu.AeroMenuGUI.IsBuiltInProtected(player))
			{
				return HarmonyControl.Continue;
			}
			...
			if (!lobbyJoinSyncGrace && clientId >= 0)
			{
				...
				{
					string rpcAction = callId == 21 && GuardOptions.SnapToAction != null ? GuardOptions.SnapToAction.Value : null;
		...
		{
			GuardPlugin.Logger?.LogWarning((object)$"Network protection ignored RPC check error for {PlayerName(player)} (client {clientId}, call {callId}): {error.Message}");
			return HarmonyControl.Continue;
		}
		finally
		{
...
internal static bool CheckShipStatusRpc(ShipStatus ship, int callId, MessageReader reader)
	{
		...
		try
		{
			...
			if (AeroMenu.AeroMenuGUI.IsBuiltInProtected(clientId) ||
				AeroMenu.AeroMenuGUI.IsBuiltInProtected(actor))
			{
				return HarmonyControl.Continue;
			}
...
internal static bool CheckVoteKickRpc(VoteBanSystem system, int callId, MessageReader reader)
	{
		...
		try
		{
			...
			if (AeroMenu.AeroMenuGUI.IsBuiltInProtected(voterClientId))
			{
				return HarmonyControl.Continue;
			}
			...
			if (AeroMenu.AeroMenuGUI.banVoteKickVoters)
			{
				AeroMenu.AeroMenuGUI.VoteBanSystemPatch.BanVoteKickVoter(voterClientId, ClientName(voterClientId), ClientName(target));
				return HarmonyControl.SkipOriginal;
			}
...
internal static void TrackClientJoined(ClientData client)
	{
		...
		GuardPlugin.Logger?.LogInfo((object)$"NetGuard client {client.Id} passed entry checks");
		LogClientSuccessDetails(client);
		GuardPlugin.Logger?.LogInfo((object)$"NetGuard client {client.Id} entered lobby");
	}
...
private static void LogClientSuccessDetails(ClientData client)
	{
		...
		try
		{
			if (PlayerLevelCache.TryGetDisplayLevel(client.Id, out uint cachedLevel))
			{
				levelText = cachedLevel.ToString();
			}
			else if (client.Character != null && PlayerLevelCache.TryGetDisplayLevel(client.Character, out uint characterLevel))
			{
...
internal static void UpdateJoinIntegrityChecks()
	{
		...
		for (int i = 0; i < ids.Count; i++)
		{
			...
			if (!ClientIdentityReady(client))
			{
				...
				GuardPlugin.Logger?.LogWarning((object)$"Network protection stopped waiting for client {clientId}: player data is still incomplete.");
				continue;
			}
