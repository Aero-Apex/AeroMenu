...
private static bool TryGetSnapshotIdentity(int clientId, out ClientIdentity identity)
	{
		identity = default;
		...
		identity = new ClientIdentity(clientId, snapshot.PlayerName, snapshot.FriendCode, snapshot.ProductUserId);
		return true;
	}
...
private static void RememberConnectionClient(string connectionKey, int clientId)
	{
		...
		if (ClientIdByConnectionKey.TryGetValue(connectionKey, out int existingClientId) && existingClientId != clientId)
		{
			...
			ConnectionKeySeenAt[connectionKey] = Time.realtimeSinceStartup;
			GuardPlugin.Logger?.LogInfo((object)$"Network protection marked connection key as ambiguous: {TrimConnectionKeyForLog(connectionKey)} ({existingClientId} -> {clientId}).");
			return;
		}
...
private static string ClientName(int clientId)
	{
		try
		{
			if (AmongUsClient.Instance != null)
			{
				...
				if (data != null)
				{
					return BanAccessList.ClientDisplayName(data, clientId);
				}
			}
		}
		catch
...
private static void Notice(string title, string detail)
	{
		...
		GuardToast.Show(title, detail, 3.2f);
	}
...
internal static void FlagLobbyTeleport(PlayerControl player, int clientId, float dist)
	{
		string action = GuardOptions.LobbyTeleportAction == null
			? "Warn"
			: GuardOptions.NormalizeNetworkProtectionAction(GuardOptions.LobbyTeleportAction.Value);
		string name = PlayerName(player);
		string title = "Lobby teleport";
		string detail = $"Snapped {dist:F1} units.";
		GuardPlugin.Logger?.LogWarning((object)$"Lobby teleport: {name} (client {clientId}) snapped {dist:F1} units.");
