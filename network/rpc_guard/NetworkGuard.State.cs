...
internal static void TrackInboundSender(InnerNetClient client, DataReceivedEventArgs eventArgs)
	{
		try
		{
			...
			if (PacketMonitor.Enabled && client != null && !client.AmHost && eventArgs != null)
			{
				PacketMonitor.RecordInboundPacket(-1, eventArgs.Message);
			}

			bool voteKickSenderTracking = AeroMenu.AeroMenuGUI.banVoteKickVoters && client != null && client.AmHost;
			bool ventKickSenderTracking = AeroMenu.AeroMenuGUI.blockVentKickExploit && client != null && client.AmHost;
			bool customRpcSenderTracking = AeroMenu.AeroMenuGUI.blockSpoofRPC && client != null && client.AmHost;
			bool unownedSpawnSenderTracking = AeroMenu.AeroMenuGUI.enableUnownedSpawnGuard && client != null && client.AmHost;
			bool overflowSenderTracking = NetIdOverflowProtectionEnabled() && client != null && client.AmHost;
			if ((!Enabled() && !voteKickSenderTracking && !ventKickSenderTracking && !customRpcSenderTracking && !unownedSpawnSenderTracking && !overflowSenderTracking) || client == null || !client.AmHost || eventArgs == null)
			{
			...
			if (!string.IsNullOrWhiteSpace(activeInboundConnectionKey))
			{
				...
				if (FloodDropConnectionUntil.TryGetValue(activeInboundConnectionKey, out float dropUntil) && nowFast <= dropUntil)
				{
					if (FloodDropClientByConnectionKey.TryGetValue(activeInboundConnectionKey, out int droppedFast) && droppedFast >= 0)
					{
						...
						if (PacketMonitor.Enabled)
						{
							PacketMonitor.RecordInboundPacket(droppedFast, eventArgs.Message);
						}
					}
					return;
				}
			...
			if (PacketMonitor.Enabled)
			{
				PacketMonitor.RecordInboundPacket(ResolveBestActiveClientId(senderClientId), eventArgs.Message);
			}
		}
		catch (Exception error)
		{
			...
			if (now - lastInboundSenderErrorAt > 2f)
			{
				...
				GuardPlugin.Logger?.LogWarning((object)$"Network protection ignored sender tracking error: {error.Message}");
			}
		}
	}
...
internal static bool CheckMessage(InnerNetClient client, MessageReader reader, SendOption sendOption)
	{
		if (!Enabled())
		{
			...
			if (client != null && reader != null && (reader.Tag == 5 || reader.Tag == 6))
			{
				...
				{
					SecurityNotice.Show("Warn", null, "Malformed GameData (non-host)", "Bad framing", -1);
					return HarmonyControl.SkipOriginal;
				}
				...
				try
				{
					...
					while (copy.Position < copy.Length && parts < MaxTrackedGameDataParts)
					{
						...
						if (partTag == 1 && client != null && !client.AmHost &&
							GuardOptions.NonHostDataDrop != null && GuardOptions.NonHostDataDrop.Value)
						{
							try
							{
								...
								if (AmongUsClient.Instance != null)
								{
										...
										!innerData.DestroyedObjects.Contains(nid))
									{
										...
										if (flood && phantom)
										{
											SecurityNotice.Show("Warn", null, "Phantom DATA flood (non-host)", $"netId {nid}", -1);
											return HarmonyControl.SkipOriginal;
										}
									}
								}
								...
								{
									SecurityNotice.Show("Warn", null, "DATA flood (non-host)", $"netId {nid}", -1);
									return HarmonyControl.SkipOriginal;
								}
							}
							catch { }
							...
							GuardOptions.NonHostDataDrop != null && GuardOptions.NonHostDataDrop.Value)
						{
							try
							{
								...
								{
									SecurityNotice.Show("Warn", null, "Oversized name RPC (non-host)", $"#{rcall} netId {rnid}", -1);
									return HarmonyControl.SkipOriginal;
								}

								if (VentRpcOutOfRange(part, rcall))
								{
									SecurityNotice.Show("Warn", null, "Invalid vent id (non-host)", $"#{rcall} netId {rnid}", -1);
									return HarmonyControl.SkipOriginal;
								}
								...
								if (rpcFlood)
								{
									...
									SecurityNotice.Show("Warn", null, what, $"netId {rnid}", -1);
									return HarmonyControl.SkipOriginal;
								}
							}
							catch { }
					...
					{
						SecurityNotice.Show("Warn", null, "Spawn flood (non-host)", "fake-owner / over cap", -1);
						return HarmonyControl.SkipOriginal;
					}
					...
					{
						SecurityNotice.Show("Warn", null, "Duplicate spawn (non-host)", "netId already live", -1);
						return HarmonyControl.SkipOriginal;
					}
					}
		...
		if (BanAccessList.TryHandleJoinMessage(client, reader))
		{
			return HarmonyControl.SkipOriginal;
		}
		...
		try
		{
			...
			if (AeroMenu.AeroMenuGUI.IsBuiltInProtected(senderClientId))
			{
				return HarmonyControl.Continue;
			}
			...
			if (tag == 5 || tag == 6)
			{
				if (!GameDataFramingValid(reader, tag))
				{
					...
					string malformedAction = GuardOptions.MalformedDataAction != null ? GuardOptions.MalformedDataAction.Value : "Warn";
					BlockMessage(senderClientId, "Malformed GameData", "Bad framing вЂ” dropped before parse.", malformedAction);
					return HarmonyControl.SkipOriginal;
				}
...
private static bool SpawnShouldDrop(MessageReader part)
	{
		if (GuardOptions.SpawnFloodGuard != null && !GuardOptions.SpawnFloodGuard.Value)
		{
			return false;
		}
