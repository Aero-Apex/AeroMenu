...
private void UpdateMenuTrackTex(Texture2D tex, Color color)
        ...
        private void InitStyles()
        {
            ...
            texBtnHover = MakeRoundedTex(64, isLightTheme ? new Color(0.92f, 0.92f, 0.92f, 1f) : new Color(0.34f, 0.34f, 0.34f, 1f), 6f);
...
private void DestroyMenuStyleTextures()
        {
            ...
            DestroyMenuTexture(ref texBtnHover);
...
private void LoadBackgroundImage()
        {
            try
            {
                string bgPath = System.IO.Path.Combine(Plugin.AeroFolder, "MenuBG.png");
                if (!System.IO.File.Exists(bgPath)) bgPath = System.IO.Path.Combine(Plugin.AeroFolder, "MenuBG.jpg");
...
private void LoadMenuCharacter()
        {
            try
            {
                string path = System.IO.Path.Combine(Plugin.AeroFolder, "Char.png");
                if (!System.IO.File.Exists(path)) path = System.IO.Path.Combine(Plugin.AeroFolder, "Char.jpg");
                if (!System.IO.File.Exists(path)) path = System.IO.Path.Combine(Plugin.AeroFolder, "MenuCharacter.png");
                if (!System.IO.File.Exists(path)) path = System.IO.Path.Combine(Plugin.AeroFolder, "MenuCharacter.jpg");
