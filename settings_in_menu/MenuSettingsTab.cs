...
private void DrawMenuSectionHeader(string title)
        {
            ...
            GUILayout.Label(GUIContent.none, menuAccentBarStyle, GUILayout.Width(4), GUILayout.Height(18));
            GUILayout.Space(8);
...
private void DrawMenuTab()
        {
            ...
            GUILayout.Label(L("Put 'Char.png' in the AeroMenu folder. It is drawn under the menu controls and does not block clicks.", "Поместите Char.png в папку AeroMenu. Картинка рисуется под контролами меню и не блокирует клики."), menuDescStyle);
            GUILayout.Space(8);
            ...
            GUILayout.Label(L("Shows the AeroMenu watermark near ping and FPS.", "РџРѕРєР°Р·С‹РІР°РµС‚ РІРѕС‚РµСЂРјР°СЂРє AeroMenu СЂСЏРґРѕРј СЃ ping Рё FPS."), menuDescStyle);
            GUILayout.Space(8);
            ...
            if (GUILayout.Button(L("Reset Menu Layout", "Сбросить расположение меню"), btnStyle, GUILayout.Height(26), GUILayout.Width(260f)))
            {
                windowRect = new Rect(100f, 100f, defaultMenuWidth, defaultMenuHeight);
                menuScale = 1f;
                SaveConfig();
                ShowNotification("<color=#00FF00>[MENU]</color> Layout reset");
            }
            GUILayout.Label(L("Restores the default window position, size and scale.", "Возвращает окно в позицию, размер и масштаб по умолчанию."), menuDescStyle);
            GUILayout.Space(8);
