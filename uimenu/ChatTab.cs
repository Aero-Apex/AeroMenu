...
namespace AeroMenu
{
    ...
    {
public static bool showChatRoles = false;
...
private void DrawChatSettingsContent()
        {
            ...
            GUILayout.Space(2);
            showChatRoles = DrawToggle(showChatRoles, L("Show Roles in Chat", "Показывать роли в чате"), 280);
            GUILayout.Space(2);
...
public static void AddPortableChatLog(PlayerControl sourcePlayer, string chatText)
        {
            ...
            try
            {
                ...
                if (AeroMenuGUI.showChatRoles && sourcePlayer != null && sourcePlayer.Data != null && sourcePlayer.Data.Role != null)
                    roleTag = AeroMenuGUI.GetChatRoleTag(sourcePlayer.Data);
                ...
                portableChatLogs.Add($"<color=#{timeColor}>[{time}]</color> <color=#{nameColor}>{safeName}</color>{roleTag}: <color=#{textColor}>{safeText}</color>");
