...
isWaitBindReviveAll || isWaitBindToggleAutoFix || isWaitBindCopyCode;
            ...
            if (!isTypingOrBinding && !gameBlockingInput)
            {
                ...
                if (bindToggleAutoFix != KeyCode.None && Input.GetKeyDown(bindToggleAutoFix))
                {
                    autoRepairSabotage = !autoRepairSabotage;
                    ShowNotification(autoRepairSabotage ? "<color=#00FF00>[AUTO FIX]</color> ON" : "<color=#FF0000>[AUTO FIX]</color> OFF");
                }
                if (bindCopyCode != KeyCode.None && Input.GetKeyDown(bindCopyCode))
                {
                    try
                    {
                        InnerNetClient cl = (InnerNetClient)AmongUsClient.Instance;
                        if (cl != null && cl.GameId != 0)
                        {
                            string code = GameCode.IntToGameName(cl.GameId);
                            GUIUtility.systemCopyBuffer = code;
                            ShowNotification("<color=#00FF00>[CODE]</color> Copied: " + code);
                        }
                        else
                        {
                            ShowNotification("<color=#FF0000>[CODE]</color> Not in a lobby");
                        }
                    }
                    catch { }
                }
            ...
            AeroAutoHostService.Tick();
            AeroAutoLobbyReturn.UpdateLogic();
            AeroBugroomFarmService.Tick();
            AeroBugroomScoutService.Tick();
            ApplyFpsLimit();
            TryAutoGhostAfterStartTick();
            TryAutoBanCustomPlatformsTick();
            UpdateUnfixableLightsState();
            TickAutoRepairSabotage();
            TickAutoBreakSabotage();
...
private static void TickFollowPlayer()
        {
            try
            {
                PlayerControl local = PlayerControl.LocalPlayer;
                PlayerControl target = FindPlayerById(followPlayerId);
                if (local == null || local.NetTransform == null || target == null || target.transform == null || target.Data == null || target.Data.Disconnected)
                {
                    followPlayer = false;
...
private static bool TryOpenGlobalChatViaNoEjectExile()
        {
            ...
            try
            {
                ...
                MeetingHud.Instance.RpcVotingComplete(emptyStates, null, false, false, 0);
                MeetingHud.Instance.RpcClose();
                return true;
            }
            catch
