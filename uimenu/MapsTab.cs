...
[HarmonyPatch(typeof(PlayerControl), nameof(PlayerControl.SetColor))]
        public static class AutoKickBugs_Patch
        {
            ...
            {
                if (!AeroMenuGUI.autoKickBugs || AmongUsClient.Instance == null || !AmongUsClient.Instance.AmHost) return;
                ...
                if (bodyColor == 18 || colorName == "???" || bodyColor >= Palette.PlayerColors.Length)
                        {
                            ...
                            {
                                AeroMenuGUI.fortegreenTimer.Remove(pid);
                                return;
                            }

                            if (!AeroMenuGUI.fortegreenTimer.ContainsKey(pid))
                            {
                                AeroMenuGUI.fortegreenTimer[pid] = Time.time + AeroMenuGUI.autoKickTimer;
                            }
                        }
                        else
                        {
...
[HarmonyPatch(typeof(VoteBanSystem), nameof(VoteBanSystem.HandleRpc))]
        public static class VoteBanSystemPatch
        {
            public static bool Prefix(VoteBanSystem __instance, byte callId, Hazel.MessageReader reader)
            {
                ...
                bool shouldBlock = AmongUsClient.Instance != null && AmongUsClient.Instance.AmHost && (AeroMenuGUI.disableVoteKicks || AeroMenuGUI.banVoteKickVoters);
                try
                {
                    Hazel.MessageReader copy = Hazel.MessageReader.Get(reader);
                    int firstClientId = copy.ReadInt32();
                    int targetClientId = copy.ReadInt32();
                    int voterClientId = AeroNetGuard.NetworkGuard.ResolveCurrentRpcSenderClientId(__instance, callId);
                    AeroMenuGUI.RegisterVoteKickEsp(targetClientId);
                    ...
                    if (AeroMenuGUI.banVoteKickVoters && AmongUsClient.Instance != null && AmongUsClient.Instance.AmHost)
                        BanVoteKickVoter(voterClientId, voterName, targetName);
                    if (shouldBlock)
                        AeroMenuGUI.ShowNotification($"<color=#FFAC1C>[VOTEKICK BLOCK]</color> {voterName} tried to vote-kick {targetName}");
                }
                catch
                {
                    if (shouldBlock)
                        AeroMenuGUI.ShowNotification("<color=#FFAC1C>[VOTEKICK BLOCK]</color> Vote-kick blocked, sender could not be resolved.");
                }
            ...
            internal static void BanVoteKickVoter(int voterClientId, string voterName, string targetName)
            {
                try
                {
                    ...
                    if (voter != null && AeroMenuGUI.IsProtectedFromAnticheat(voter)) return;
                    if (voter == null && AeroMenuGUI.IsProtectedFromAnticheat(voterClientId)) return;

                    AeroNetGuard.NetworkGuard.BanClient(voterClientId, "Vote-kick attempt", CleanVoteName(targetName));
                    AeroMenuGUI.ShowNotification($"<color=#FF4444>[VOTE BAN]</color> {CleanVoteName(voterName)} banned for vote-kick.");
                }
                catch { }
            }
            ...
            private static void ShowVoteKickChatInfo(string voterName, string targetName)
            {
                ...
                AeroMenuGUI.ShowNotification(message);
            }
...
[HarmonyPatch(typeof(ShhhBehaviour), nameof(ShhhBehaviour.PlayAnimation))]
        public static class SkipShhh_Perfect_Patch
        {
            ...
            {
                AeroMenuGUI.MarkCurrentGameIntroShhhSeen();
                AeroMenuGUI.NotifyAutoChatEveryoneShhhSeen();
            }

            public static void Postfix(ShhhBehaviour __instance, ref Il2CppSystem.Collections.IEnumerator __result)
            {
                if (!AeroMenuGUI.skipShhhAnim || __instance == null || __result == null) return;
...
[HarmonyPatch(typeof(CosmeticsCache), nameof(CosmeticsCache.GetNameplate))]
        public static class CosmeticsCache_GetNameplate_RoleSkip_Patch
        {
            ...
            {
                if (!AeroMenuGUI.skipRoleIntroAnim || MeetingHud.Instance == null)
                    return true;

                try
                {
                    if (AeroMenuGUI.meetingNameplate == null)
                    {
                        AeroMenuGUI.meetingNameplate = ScriptableObject.CreateInstance<NamePlateViewData>();
                        AeroMenuGUI.meetingNameplate.Image = null;
                    }

                    __result = AeroMenuGUI.meetingNameplate;
                    return false;
                }
                catch
                {
...
[HarmonyPatch(typeof(CosmeticsCache), nameof(CosmeticsCache.Destroy))]
        public static class CosmeticsCache_Destroy_RoleSkip_Patch
        {
            public static void Postfix()
            {
                ...
                {
                    if (AeroMenuGUI.meetingNameplate != null)
                        Object.Destroy(AeroMenuGUI.meetingNameplate);
                }
                catch { }

                AeroMenuGUI.meetingNameplate = null;
            }
        }
...
[HarmonyPatch(typeof(IntroCutscene), "CoBegin")]
        public static class IntroCutscene_CoBegin_RoleSkip_Patch
        {
            ...
            {
                __state = null;
                if (!AeroMenuGUI.skipRoleIntroAnim || __instance == null || !AeroMenuGUI.IsHideAndSeekMode())
                    return;
            ...
            {
                if (!AeroMenuGUI.skipRoleIntroAnim)
                    return;
                ...
                try
                {
                    Plugin.Instance?.Log?.LogMessage((object)$"[ANIM] Role intro skip applied: {(AeroMenuGUI.IsHideAndSeekMode() ? "HideAndSeek" : "Normal")}");
                }
                catch { }
...
[HarmonyPatch(typeof(KillOverlay), nameof(KillOverlay.ShowKillAnimation), new Type[] { typeof(NetworkedPlayerInfo), typeof(NetworkedPlayerInfo) })]
        public static class KillOverlay_ShowKillAnimation_Info_Skip_Patch
        {
            ...
            {
                return !AeroMenuGUI.skipKillAnimation;
            }
        }
