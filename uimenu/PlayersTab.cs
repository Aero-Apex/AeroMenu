...
private void DrawPlayersTab()
        {
            ...
            if (target != null && target.Data != null)
            {
                ...
                if (DrawFixedMenuButton("KILL", btnStyle, playerActionThirdWidth, playerActionButtonHeight))
                {
                    ...
                    if (local != null && local.NetTransform != null)
                    {
                        if (AmongUsClient.Instance != null && AmongUsClient.Instance.AmHost)
                            TryHostAeroMurderPlayer(target);
                        else
                        {
                ...
                if (DrawFixedMenuButton("VOTE", activeTabStyle, playerActionHalfWidth, playerActionButtonHeight))
                {
                    if (ForceCastVote(target.PlayerId, false))
                        ShowNotification($"<color=#00FF00>[VOTE]</color> Voted for <b>{target.Data.PlayerName}</b>");
                    else
                ...
                if (DrawFixedMenuButton("SKIP VOTE", btnStyle, playerActionHalfWidth, playerActionButtonHeight))
                {
                    if (ForceCastVote(0, true))
                        ShowNotification("<color=#00FF00>[VOTE]</color> Vote skipped.");
                    else
                        ShowNotification("<color=#FF0000>[VOTE]</color> Meeting required.");
                }
