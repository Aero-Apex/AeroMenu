...
private static void EnsurePlatformBanListLoaded()
        {
            try
            {
                if (string.IsNullOrEmpty(platformBanListPath))
                    platformBanListPath = System.IO.Path.Combine(Plugin.AeroFolder, "AeroPlatformBanList.txt");
...
private void DrawSelfSpoof()
        {
            ...
            const float taskCardHeight = 135f;
            ...
            if (GUILayout.Button("Complete", btnStyle, GUILayout.Height(24)))
                CompleteLocalTasks();
            GUILayout.Space(4);
            autoCompleteTasks = DrawCompactToggle(autoCompleteTasks, "Auto Complete", Mathf.RoundToInt(sideWidth - 16f));
            GUILayout.FlexibleSpace();
            GUILayout.EndVertical();
...
internal static bool CompleteLocalTask(NormalPlayerTask task)
        {
...
[HarmonyPatch(typeof(PlayerBanData), nameof(PlayerBanData.BanPoints), MethodType.Setter)]
        public static class RemoveDisconnectPenalty_Patch
        {
            ...
            {
                if (!AeroMenuGUI.removePenalty) return true;
...
[HarmonyPatch(typeof(DisconnectPopup), nameof(DisconnectPopup.DoShow))]
        public static class DisconnectPopup_CopyRoomCode_Patch
        {
            public static void Postfix(DisconnectPopup __instance)
            {
                try
                {
                    if (!AeroMenuGUI.TryCopyRoomCodeToClipboard(false)) return;
...
[HarmonyPatch(typeof(AmongUsClient), nameof(AmongUsClient.ExitGame))]
        public static class AmongUsClient_ExitGame_CopyRoomCode_Patch
        {
            ...
            {
                AeroMenuGUI.TryCopyRoomCodeToClipboard(true);
            }
        }
...
[HarmonyPatch(typeof(GameStartManager), nameof(GameStartManager.Start))]
        public static class ShowLobbyTimer_Patch
        {
            ...
            {
                if (!AeroMenuGUI.alwaysShowLobbyTimer) return;
                ...
                if (!AeroMenuGUI.rainbowLobbyTimer && HudManager.Instance != null)
                {
        ...
        public static class LobbyTimerColor_Patch
        {
            ...
            public static void Postfix(TimerTextTMP __instance)
            {
                try
                {
                    ...
                    if (AeroMenuGUI.alwaysShowLobbyTimer && AeroMenuGUI.rainbowLobbyTimer)
                    {
                        if (!rainbowApplied)
                        {
                            normalColor = __instance.text.color;
            ...
            public static void Postfix(GameStartManager __instance)
            {
                try
                {
                    ...
                    bool rainbow = inLobby &&
                                   AeroMenuGUI.alwaysShowLobbyTimer &&
                                   AeroMenuGUI.rainbowLobbyTimer;
                    ...
                    if (!rainbow)
                    {
                        ...
                        if (applied && inLobby && AeroMenuGUI.alwaysShowLobbyTimer &&
