...
namespace AeroMenu
{
    internal sealed class RepeatedLogFilter : ILogListener
    {
        ...
        private static bool IsOwnLog(object sender, LogEventArgs eventArgs)
        {
            ...
            if (!string.IsNullOrEmpty(src))
                return src.Contains("Aero") || src.Contains("NetGuard");

            string msg = eventArgs.Data as string ?? eventArgs.Data?.ToString();
            return msg != null && (msg.Contains("Aero") || msg.Contains("NetGuard") || msg.StartsWith("Raw RPC "));
        }
        ...
        private static bool IsKnownVerboseNoise(LogEventArgs eventArgs)
        {
            ...
            if ((eventArgs.Level & LogLevel.Warning) == 0 &&
                (message.StartsWith("Registered mono type AeroMenu.", System.StringComparison.Ordinal) ||
                 message.StartsWith("Registered mono type AeroNetGuard.", System.StringComparison.Ordinal)))
                return true;
